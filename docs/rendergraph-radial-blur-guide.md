...
## 9. Render Graph の基礎（面接前の確認用）

本実装で触った範囲を、仕組みの側から整理したもの。

### 9-1. 位置づけ

**Render Graph は URP 専用ではない。** `com.unity.render-pipelines.core` の
`UnityEngine.Rendering.RenderGraphModule` にある **SRP 共通の仕組み**。
HDRP は以前から使っており、URP が全面移行したのが Unity 6（URP 17）。
「URP の API 群」と説明すると誤り。

**Renderer Feature と Volume はセットではない。**
Volume は単なるパラメータの供給源であり、`[SerializeField]` の値を直接使う Feature も普通にある。
今回 Volume を使ったのは「ランタイムにゲームの状態から制御したかった」という**設計判断**であって、
必然ではない。この区別は説明できるようにしておく。

### 9-2. 宣言 → コンパイル → 実行

**`RecordRenderGraph` は何も実行していない。宣言しているだけ。**

```
1. Record   … 全 Feature の RecordRenderGraph が呼ばれ、パスと依存関係が「宣言」される
2. Compile  … 宣言から依存グラフ(DAG)を構築し、
               カリング・リソース寿命計算・メモリ割り当て・パス統合を決定
3. Execute  … SetRenderFunc に登録した関数が順に呼ばれ、実際に GPU コマンドが積まれる
```

`AddRasterRenderPass` が行っているのは:

- グラフ上に**パスのノードを1つ作る**
- **`PassData` のインスタンスを渡す**（プール由来。毎フレーム new されない）
- リソース宣言用の **`builder` を返す**
- `using` ブロックを抜けた時点でパスが確定してグラフに登録される

**この時点では GPU コマンドは1つも積まれていない。**

本実装で踏んだ2件は、どちらもこの構造から直接来ている。

| 現象 | 原因 |
|---|---|
| `static` ラムダが必要（§3 Step 4） | 記録と実行が分離しているため、外部状態を掴むと実行時に別の値を読む |
| 書き戻しパスが無いとパスごと消えた（§3 Step 3） | Compile フェーズのカリング |

### 9-3. builder の宣言が持つ意味

`UseTexture` / `SetRenderAttachment` は単なる入出力指定ではなく、
**グラフに依存辺を張る宣言**。これにより Compile フェーズが4つを自動で行う。

**1. データフローの確定（順序の自動決定ではない）**

**Render Graph はパスを並べ替えない。** 実行順は**記録順からカリングされたものを除いたもの**。
宣言が決めるのは順序ではなく「**どのリソースのどのバージョンを読むか**」というデータフロー。

Render Graph はリソースに**バージョン**を持たせており、書き込みのたびにバージョンが進む。
`UseTexture` は**その時点で最新のバージョン**に結び付く。

```
RadialBlur : read source(v0) → write temp(v0→v1)
Composite  : read temp(v1)   → write source(v0→v1)
```

したがって **`RecordRenderGraph` はデータが流れる順に書く**のが原則。詳細は §9-5。

**2. パスのカリング**
出力を誰も読まないパスは消える。本実装で書き戻しパスが無かったとき、
`temp` の消費者が居らずブラーパスごと消えた。

**3. リソースの寿命とメモリエイリアシング**
**`TextureHandle` はテクスチャの実体ではなく「約束手形」。**
実際のメモリは Compile 後にプールから割り当てられ、
寿命が重ならない別のリソースとメモリを共有（エイリアス）する。
旧 API で必要だった `RTHandle` の手動 `Alloc` / `Release` が不要なのはこのため。

**4. ネイティブレンダーパスへの統合（モバイルで効く）**
`SetRenderAttachment` は「ラスタライザの出力先（フレームバッファのアタッチメント）」という宣言。
ランダムライトではなくアタッチメントだと分かっているため、条件が合えば
**複数パスを1つのネイティブレンダーパスに統合し、タイルメモリ上で完結できる**。
TBDR なモバイル GPU では帯域が大きく変わる。

なお `SetRenderAttachment` の既定は `AccessFlags.Write` で、これは
「**全面を上書きするので既存内容の読み込み（Load）は不要**」という意味を持つ。
`ReadWrite` にすると Load が発生する。フルスクリーン Blit は全面を塗るので `Write` が正しく、
これもタイルメモリの Load を省ける要因になる。

### 9-4. パスの3種類

| | 用途 | 特徴 |
|---|---|---|
| `AddRasterRenderPass` | 描画・Blit | `RasterCommandBuffer` のみ。**制約が強い代わりにパス統合の対象になる** |
| `AddComputePass` | コンピュートシェーダー | |
| `AddUnsafePass` | 逃げ道 | 通常の `CommandBuffer` が使える。`SetRenderTarget` を手で叩ける代わりに**統合対象外** |

「制約を受け入れると最適化してもらえる」という設計。
**`Unsafe` を使わず `Raster` で書けたこと自体が、パイプラインの意図に沿った実装である証明**になる。

### 9-5. 本実装の2パスが何をしているか

| パス | 読む (`UseTexture`) | 書く (`SetRenderAttachment`) | 内容 |
|---|---|---|---|
| `RadialBlur` | `source`（フル解像度のカメラカラー） | `temp`（縮小バッファ） | ブラーを適用しつつ縮小 |
| `RadialBlurComposite` | `temp` | `source` | バイリニア拡大で書き戻す |

**同一テクスチャの読み書きはできない**ため、`temp` を経由する構成は原理的に必要。
したがって**フル解像度の書き込み 1 回はどうやっても残る**。

#### 記録順を入れ替えると壊れる

2つのパスの記録順を逆にすると、リソースのバージョンが変わって**別のグラフになる**。

```
Composite  : read temp(v0)   → write source(v0→v1)   ← 誰も書いていない temp を読む
RadialBlur : read source(v1) → write temp(v0→v1)     ← Composite の出力を読む
```

- **Composite が未初期化の `temp` を読む。** `clearBuffer = false` なので、プールから
  割り当てられたメモリの中身（前フレームに別リソースが使っていた内容）がそのまま出る
- **`temp(v1)` を消費するパスが無いため RadialBlur がカリングされる。**
  書き戻しパスを書き忘れたときとまったく同じ現象

Render Graph が自動で行うのは**カリング・リソース寿命管理・パス統合**であって、
**パスの並べ替えではない。** この区別は面接で問われやすい。

**実際に入れ替えて確認済み: 画面が真っ暗になり、UI だけが表示された。**

真っ暗になったのは `temp` に割り当てられたプールのメモリがたまたまゼロだったため。
**「未定義の内容」であって「必ず黒」ではない**（前フレームの別リソースの内容が見えることもある）。
再現性のない挙動である点に注意。

UI だけ残ったのは §8 の内容の裏付けにもなる。
Screen Space Overlay の UI は SRP のカメラループ外で描画されるため、
カメラカラーがどうなろうと影響を受けない。**1回の実験で2つのことが同時に確認できる。**

Render Graph Viewer で見ると `RadialBlur` が消えているのが可視化される。
カリングが起きている様子のキャプチャは資料としても使える。

#### 順序の制御は2階層ある

| 何の順序か | 制御方法 |
|---|---|
| **1つの Feature 内の複数パス** | `RecordRenderGraph` 内の**記述順** |
| **他の Feature や URP 組み込みパスとの相対順序** | `RenderPassEvent` |

`RadialBlur` → `Composite` は前者で、コードの並び順がそのまま制御になる。
順序を指定する API を呼ぶわけではない。

`BeforeRenderingPostProcessing` を指定しているのは後者で、
アウトラインなど他の Feature を追加したときにどちらが先に走るかを決めている。
**同じ `RenderPassEvent` を持つ Feature 同士の順序は、Renderer アセットに登録した順**になる
（§6 のアウトライン最適化で両方を有効にする場合に効いてくる）。

### 9-6. 仕組みと計測結果のつながり（最も強い話）

> 同一テクスチャの読み書きができないので、書き戻しパスが原理的に必要。
> だからフル解像度の書き込み 1 回はどうやっても残る。
> 実測ではそれが **0.65ms の固定コスト**として現れ、
> 解像度を 1/4 まで落としても総コストがそこで頭打ちになった（§4-5 発見2・発見3）。

**仕組みの理解 → 実装上の制約 → 実測値、が一本の線でつながる。**

### 9-7. 30秒で答えるなら

> Render Graph は、レンダーパスとその入出力リソースを**宣言**すると、
> フレームごとに依存グラフを構築して、**使われないパスのカリング・
> リソースの寿命管理とメモリの再利用・モバイル向けのレンダーパス統合**を自動で行う仕組みです。
> 記録と実行が分離しているので、パスに渡すデータは `PassData` 経由にする必要があります。
> なお**並べ替えはしないので、記録順がそのままデータフローになります。**
> 今回は `activeColorTexture` を読んで縮小バッファに書くパスと、
> それをカメラカラーに戻すパスの2本を宣言しました。
> **書き戻しパスを書き忘れたときは出力を消費するパスが無く、
> グラフごとカリングされて何も表示されませんでした。**

最後の失敗談まで入れると、暗記ではなく体験として理解していることが伝わる。

### 9-8. TBDR とレンダーパス統合はデスクトップでも重要か

参考: [CyberAgent - RenderGraphTutorial](https://github.com/CyberAgentGameEntertainment/RenderGraphTutorial)

**Render Graph の恩恵をひとまとめにせず、2種類に分けて考える。**

| 機能 | TBDR（モバイル） | IMR（従来のデスクトップ） |
|---|---|---|
| ネイティブレンダーパス統合 | 極めて重要。タイルメモリ内で完結し DRAM 往復が消える | ほぼ効果なし |
| Load/Store アクション最適化 | 重要。タイルの読み込み・書き戻しを省ける | ほぼ効果なし |
| **パスのカリング** | 効く | **同じだけ効く** |
| **リソース寿命管理・メモリエイリアシング** | 効く | **同じだけ効く**（VRAM 削減） |
| **依存関係の明示による検証・可視化** | 効く | **同じだけ効く** |

上2つが TBDR 固有、下3つはアーキテクチャ非依存。

そもそも Render Graph は **HDRP が先に採用した仕組み**であり、
当初の動機は TBDR ではなく「複雑なフレームのリソース管理を自動化すること」だった。
モバイル向けに生まれた仕組みではない。

本実装で効いたのも下3つの側。パスカリングの件も 0.65ms の固定コスト特定も TBDR とは無関係。

#### 「非モバイル = IMR」はもう成り立たない

- **Apple Silicon（M1〜M4）は TBDR。** デスクトップの Mac が TBDR という状況
- **NVIDIA は Maxwell 以降タイルドキャッシング**を採用（三角形をタイルに分類して L2 に載せる）
- **AMD も RDNA 世代で DSBR**（Draw Stream Binning Rasterizer）を持つ

完全な TBDR ではないため Load/Store 省略ほどの効果はないが、
「デスクトップだから無関係」と言い切るのは現在では不正確。

#### 本実装のブラーは、そもそも統合できない

**レンダーパス統合が成立する条件は「後続パスが同じピクセル座標しか読まないこと」。**
タイルメモリに乗っているのは自分の担当タイルだけなので、
隣のピクセルを読もうとした瞬間に成立しない。
Unity では `SetInputAttachment` で宣言するフレームバッファフェッチがこれに当たる。

**ブラーは定義上、近傍ピクセルを読む。** したがって `RadialBlur` と `Composite` は
**モバイルであっても統合されない。** 必ず一度メモリに書き出す必要がある。
`UseTexture`（通常のサンプリング読み）を使っているのは、それ以外に選択肢が無いため。

モバイルでこのエフェクトが得をするのは統合ではなく:

- **解像度を落とすこと**（タイルあたりの処理量と帯域が直接減る）
- **`AccessFlags.Write` による Load の省略**（全面を上書きするのでタイルの読み込みが不要）

#### 本プロジェクトは Android ターゲット

`AndroidTargetArchitectures: 5`（ARMv7+ARM64）でビルド設定済みのため TBDR は他人事ではない。

デスクトップで得た「1/4 解像度が最適」という結論は、帯域が桁違いに狭いモバイルでは
**さらに有利に出る可能性が高い**。逆に「1/4 より下げても改善しない」という頭打ちは
ソーステクスチャへのキャッシュ効率が原因なので、**モバイルでは現れ方が変わる可能性がある**。

#### 面接での答え方

> Render Graph の恩恵は2種類あって、レンダーパス統合と Load/Store 最適化は TBDR 固有ですが、
> パスのカリング・リソースのメモリエイリアシング・依存関係の可視化はアーキテクチャに依存しません。
> そもそも Render Graph は HDRP がデスクトップ向けに先行採用した仕組みで、
> 動機はリソース管理の自動化でした。
>
> ただし Apple Silicon は TBDR ですし、NVIDIA も Maxwell 以降はタイルドキャッシングなので、
> 「デスクトップだから無関係」とは言い切れません。
>
> 今回作ったブラーに限れば、**近傍ピクセルを読むのでそもそも統合の対象外**です。
> モバイルで効くのは統合ではなく、解像度削減による帯域低減と、全面上書きによる Load の省略のほうです。

「TBDR 向けの機能だから関係ない」と切り捨てず、**機能を分解した上で
自分のケースには何が効いて何が効かないかを説明できる**のが最も良い答えになる。

---

## 10. 次の題材の候補

### 10-1. Render Graph の用途は大きく2系統

| | 内容 | 本実装 |
|---|---|---|
| **フルスクリーン Blit 系** | 画面全体を1枚のテクスチャとして加工する | 実施済み |
| **シーン描画系（RendererList）** | シーンのオブジェクトを**もう一度描く** | **未着手** |

**Renderer Feature の本来の半分は後者。**
ポストエフェクトだけやると、こちらに一度も触れずに終わる。**次に学ぶ価値が最も高い。**

使う API が別物になる。

```csharp
// Blit 系（本実装）
Blitter.BlitTexture(ctx.cmd, source, scaleBias, material, 0);

// シーン描画系
var list = renderGraph.CreateRendererList(desc);   // 描画対象を絞り込む
builder.UseRendererList(list);
ctx.cmd.DrawRendererList(list);
```

`RendererListDesc` でレイヤーマスク・レンダーキュー・シェーダータグ・ソート順を指定して
「何を描くか」を絞り込む。カリングやソートは URP に任せられる。

代表例:

- **壁越しシルエット表示（X-Ray）** — `ZTest Greater` で敵味方を単色描画
- **オブジェクト単位のアウトライン** — 対象だけマスクに描き、後段でシルエット化
- **プレーナーリフレクション** — 別視点からシーンをもう一度描く
- **カスタムの深度・法線プリパス、ID バッファ**
- **デカール、ステンシルを使った切り抜き**
...
| 用途 | 内容 |
|---|---|
| **コンピュートパス**（`AddComputePass`） | GPU カリング、パーティクル、自動露出用ヒストグラム。**Raster 以外のパス**を扱う経験は別枠で価値がある |
| **グローバルテクスチャの公開** | `builder.SetGlobalTextureAfterPass()` で他シェーダーから参照可能にする。風テクスチャ、自作の影マスクなど |
| **デバッグ可視化** | オーバードロー、ライトコンプレキシティ、法線可視化。実装が軽く実用性がある |
| **条件付きレンダリング** | 品質設定に応じてパスを積む/積まない。本実装の `IsActive()` の拡張 |

### 10-3. 本プロジェクトでの推奨: 壁越しシルエット表示（X-Ray）

1. **RendererList を使う** — 未経験の系統に触れられる
2. **ゲーム的な必然性がある** — ロックオン機構が既にあるので、遮蔽された敵の表示は自然な機能
3. **本実装と対になる** — 「ポストエフェクト」と「シーン再描画」で Renderer Feature の両輪が揃う

既存の `OutlineRendererFeature` をオブジェクト単位のアウトラインに作り直すのも同じ理由で良い題材
（§6 参照）。

### 10-4. レンダーパス統合を体験したい場合

**別枠の小さな実験として切り出す。** 題材選びに悩むより、確実に統合が起きる構成を狙い撃ちする。

**同じピクセルしか読まないポストエフェクトを2つ連鎖させ、2通りで実装して比較する。**

| 実装 | 結果 |
|---|---|
| `UseTexture`（通常のサンプリング読み） | パスが分かれる |
| `SetInputAttachment`（フレームバッファフェッチ） | **統合される** |

題材はビネット → 色調変換のような、**近傍を一切読まない処理**なら何でもよい。
Render Graph Viewer と Frame Debugger でパス数が変わるのが見える。

**デスクトップでは GPU 時間はほぼ変わらないはず。**
意味が出るのは Android 実機で帯域を測ったときなので、
そこまでやると「統合が効くのはどの環境か」を実測で語れるようになる。

教科書的な統合例は **Deferred のライティングパス**。
G-Buffer を input attachment として読むので、まさに同ピクセルアクセス。
URP の Deferred が実際にそうしているので、Frame Debugger で覗くと参考になる。
