...
|---|---|---|
| エフェクト OFF | **6.0** | — |
| 1/8 解像度・48タップ | **6.1** | 約 **0.1 ms** |
| 1/1 解像度・48タップ | **9.7** | 約 **3.6 ms** |

**ピクセル数 1/64 に対してコストが約 1/36。ほぼ比例している。**

#### 中間バッファのフォーマット（先に確定させること）
...
**つまりアルファは最初から存在せず、「中間バッファを R11G11B10 に落とす」最適化は既に適用済み。
ここに削る余地はない。**
...
フル解像度48タップは 3840×2160×48 ≈ **4億回**のテクスチャフェッチ。
これが 3.6ms という数字と桁が合う。
コストがピクセル数にほぼ比例している点と合わせると、
**帯域律速というより「総フェッチ数（ピクセル数 × タップ数）」に比例する fetch 律速**と考えられる。

**この解釈が正しいなら、サンプル数もフル解像度では大きく効くはず。**

#### 注意: サンプル数のスイープは解像度を固定して行うこと

`downsample` のデフォルトは `2`（= 1/4）。
**1/4 のままサンプル数を 4→48 に振ると、960×540×48 ≈ 2500万フェッチ止まりで
差が 1ms 程度にしかならない。これは理論通りであり、「サンプル数は効かない」証拠にはならない。**

「解像度のほうが重要」という結論自体は正しいが、
理由を「サンプル数は効かないから」とすると根拠が弱く、突っ込まれると崩れる。

#### ボトルネックを確定させる実験2つ

**1. `downsample = 0`（フル解像度）固定でサンプル数 4 → 48**

fetch 律速なら 4タップ = 3300万、48タップ = 4億で **12倍の差**が出るはず。
3.6ms の大半がタップに比例するなら、4タップ時は 0.3ms 程度まで落ちる。
確認できれば「両方効く。ただしピクセル数のほうが桁で効くので支配的」という、
より正確で強い結論になる。

**2. `m_HDRColorBufferPrecision` を `_64Bits` に変更**

減らす方向は既に限界なので、増やす方向で感度を見る。4B/px → 8B/px でコストがどう動くか。
...
**この1回でボトルネックが確定する。** 予想は「あまり変わらない（fetch 律速）」。
予想を書いてから測ると計測表としての価値が上がる。

#### 合成パスが削減の下限を決めている

1/8 にしても**合成パスはフル解像度でカメラカラーに書き戻す**ため、
そのコストは必ず残る（実測では 0.1ms 程度と、当初の見積もりより小さかった）。
これが「1/8 にしても 0 にはならない」理由。
