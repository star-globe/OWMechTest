...
### 4-5. 実測結果と考察（4K / デスクトップ）

§4-4 の通り負荷条件を作った結果、Profiler の GPU モジュールで有意差が取れた。

**計測条件:** 3840×2160 / サンプル数 48 / Profiler GPU モジュール
...
解像度固定でサンプル数を 4 → 48 に振った場合の差は **1ms 程度**にとどまった。

**結論: 解像度のほうがサンプル数より支配的。**

#### なぜ解像度が支配的なのか（メカニズム）

本プロジェクトは HDR が ON（`m_SupportsHDR: 1`）なので中間バッファは RGBA16F = **8 バイト/px**。

| | ピクセル数 | 帯域（読 8B + 書 8B） |
|---|---|---|
| フル解像度 1パス | 3840×2160 = 8.3M | 約 **133 MB** |
| 1/8 解像度 1パス | 480×270 = 0.13M | 約 **2 MB** |

ブラーパスと合成パスで2回。一般的な帯域（100GB/s 前後）で割ると 2〜3ms のオーダーになり、
**実測差 3.5ms とほぼ一致する。**

**このエフェクトは ALU 律速でもタップ数律速でもなく、メモリ帯域律速。**

サンプル数が効かない理由も同じ枠組みで説明できる。ラジアルブラーのサンプルは
中心方向へ数テクセルずつ進むだけなので、隣接ピクセルがほぼ同じテクセルを読む。
**テクスチャキャッシュのヒット率がほぼ 100%** で、タップを増やしても実メモリ traffic はほとんど増えない。

実測とも整合する。4→48 タップで 1ms ということは 1タップあたり約 0.023ms、
48タップ分でも 1.1ms 程度。残りの約 2.4ms は**タップ数に依存しないピクセル単位の固定コスト**
（ベースサンプルの読み込み・書き込み・ROP）。固定コストのほうが大きい。

これは「サンプル数を削るより解像度を落とすほうが効く」という主張の、
単なる観測ではない**メカニズムの説明**になる。「なぜ？」に答えられる状態にしておく。

#### 帯域律速と分かった上での次の一手

削るべきは帯域。解像度以外にもう1つノブがある。
中間バッファはカメラカラーの記述子をコピーしているため RGBA16F だが、ブラー結果にアルファは不要。
**R11G11B10（4 バイト/px）に落とせば帯域が半減する。**

```csharp
var desc = renderGraph.GetTextureDesc(source);
desc.width  >>= volume.downsample.value;
desc.height >>= volume.downsample.value;
desc.format = GraphicsFormat.B10G11R11_UFloatPack32;   // 8B → 4B
```

フル解像度のまま試すと効果が読み取りやすい。理屈通りなら 9.7ms から 1〜2ms 落ちるはず。
**予想を立ててから測る**ことで、計測表としての説得力が出る。

#### 合成パスが削減の下限を決めている

1/8 にしても**合成パスはフル解像度でカメラカラーに書き戻す**ため、
4K 書き込み 66MB 分（約 0.7ms）は必ず残る。これが「1/8 にしても 0 にはならない」理由。

§3 Step 3 の「パス数は 2 が下限」が、ここで数字として現れている。
**下限があることを認識した上で測っている**点も記録に残す。
