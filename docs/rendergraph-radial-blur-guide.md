...
両軸が効くと分かったので 2 次元で埋める。全マスは不要。
**ds0 と ds3 の行、48タップの列**が埋まれば主張は成立する。
...
**予想を書いてから測る**ことで、計測表としての価値が上がる。

#### 結論の書き方

> コストは総フェッチ数（ピクセル数 × タップ数）に比例する。**両軸とも効く**が、
> 解像度は 1 段下げるだけで 4 倍効くのに対し、タップ数は 48→4 でも 12 倍止まり。
> **画質劣化あたりの削減効率で解像度が有利**なので、
> まず解像度を落とし、その上でタップ数を詰めるのが正しい順序。
