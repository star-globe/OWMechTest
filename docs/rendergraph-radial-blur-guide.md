...
**Canvas の下で作ると UI レイヤーを引き継いでここで詰まる**（§4-6 で実際に踏んだ）。
...
## 4-6. トラブルシュート: `IsActive()` が false のまま
...
- [ ] Volume の GameObject が Volume Mask 外のレイヤー（UI 等）にあり、カメラに拾われない（§4-6）
...
- [ ] `overrideState` が false で、`intensity.value` を書き換えても反映されない（§4-6）
- [ ] ドライバがブースト非入力時に毎フレーム 0 で上書きし、手動テストができない（§4-6）
- [ ] Play 中に Project ウィンドウのプロファイルアセットを編集し、クローン側に反映されない（§4-6）
